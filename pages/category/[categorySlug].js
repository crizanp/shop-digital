...
export default function CategoryPage({ initialPackages, category, categories, initialPagination }) {
  ...
  const loadPage = async (page) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/packages?categoryId=${category._id}&page=${page}&limit=9`);
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    const packagesRes = await fetch(`${baseUrl}/api/packages?categoryId=${category._id}&page=1&limit=9`);
